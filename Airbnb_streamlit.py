...
def datafr():
    df= pd.read_csv('D:/Data science projects/Airbnb Analysis/Airbnb_analysis_finaldata')
    return df
...
if select == "Home":
    ...
    image1= Image.open("D:/Data science projects/Airbnb Analysis/airbnb.jfif")
    st.image(image1)
...
if select == "Data Exploration":
    ...
    with tab2:
        ...
        def datafr():
            df_a= pd.read_csv('D:/Data science projects/Airbnb Analysis/Airbnb_analysis_finaldata')
            return df_a
    ...
    with tab3:
        ...
        df_val_sel_corr= df_val_sel.drop(columns=["listing_url","name", "property_type",                 
                                            ...
                                            "host.host_url","host.host_name", "host.host_location",                   
                                            "host.host_response_time", "host.host_thumbnail_url",            
                                            "host.host_response_rate","host.host_is_superhost","host.host_has_profile_pic" ,         
                                            "host.host_picture_url","host.host_neighbourhood",
                                            "host.host_identity_verified",
                                            "address.street", "address.suburb", "address.government_area", "address.market",                        
                                            "address.country", "address.country_code","address.location.type","address.location.is_location_exact"
                                            ]).corr()
        ...
        fig_3= px.bar(df_val_sel_rt, x="address.government_area", y= ["host.host_is_superhost","host.host_neighbourhood","cancellation_policy"], title="GOVERNMENT_AREA",
                    hover_data= ["guests_included","address.location.type"], barmode='group', color_discrete_sequence=px.colors.sequential.Rainbow_r,width=1000)
